...
let grpNameBtn = document.getElementById('group-name');
let grpArea = document.getElementById('grouping-area');
let groupInput = document.getElementById('group-input');
let numberList = document.getElementById('number-list');
let grnBtn = document.getElementById('grn-btn');
let removeBtn = document.getElementById('remove-number');
let count = -1
let nameUnits = [];
let unitCount = -1
...
grnBtn.addEventListener('click', () =>{
    let grnInput = document.getElementById('grn').value;
    for (let i=0; i<grnInput; i++) {
        let Coin = Math.floor(Math.random() * 2);
        let Randomizer = Math.floor(Math.random() * randomMaleNames.length);
        count = count + 1;
        ...
        if (Coin === 0) {
            ...
            ffs.innerText = randomMaleNames[Randomizer] + ' ' + randomLastNames[Randomizer]
        }

        const nmbr = document.createElement('p')
        if (count >= 10) {
            nmbr.innerText = count
        ...
        nameList.appendChild(ffs);
        numberList.appendChild(nmbr)
    }
})

nameInput.addEventListener('keypress', ee => {
    if(ee.keyCode === 13) {
        addNameBtn.click();
    }
...
grpNameBtn.addEventListener ('click', function() {

    let grpSet = groupInput.value;
    let nameLength = document.querySelectorAll('.all-names');
    let divCount = 0

    let groupCounter = 0
    let grpTexts = document.querySelectorAll('.grpt');
    let grpSectionNumbers = document.querySelectorAll('.ppn');
    let grpSections = document.querySelectorAll('.pp');
    let grpNumbers = document.querySelectorAll('.grpn');

    console.log(grpNumbers)

    if (typeof grpNumbers === "undefined") {
        grpNumbers = 0
    }
    ...
    for (let i = 0; i < nameLength.length; i++) {
        ...
        nameUnits.push(unitCount)
        if (unitCount >= nameLength.length - 1) {
            unitCount=-1
        }
    }

    console.log(unitCount)
    console.log(nameUnits)


    if (grpNumbers.length >= 1) {
        for (let i = 0, nameTemp = nameUnits; i < nameLength.length; i++) {
            let Randomizer = Math.floor(Math.random() * nameTemp.length);
            groupCounter = groupCounter + 1
            if (groupCounter >= grpSet) {
                groupCounter = 0
            }
            let getName = document.getElementById('name' + nameTemp[Randomizer]); // Selector random
            let getNumber = document.getElementById('number' + nameTemp[Randomizer]);
    
            numberList.appendChild(getNumber);
            nameList.appendChild(getName); // Send to that random grp
    
            nameTemp.splice(Randomizer, 1);
        ...
        grpTexts.forEach(e => e.remove());
        grpNumbers.forEach(e => e.remove());
        grpSectionNumbers.forEach(e => e.remove());
        grpSections.forEach(e => e.remove());

    }



    for (let i = 0; i < grpSet; i++) {
        divCount = divCount + 1;

        let grpText = document.createElement('p')
        grpText.classList = 'grpt';
        grpText.innerText = 'Group'
        grpArea.appendChild(grpText);

        let grpNumber = document.createElement('p')
        grpNumber.classList = 'grpn';
        grpNumber.innerText = '---' + divCount + '---';
        grpArea.appendChild(grpNumber);

        let grpSectionNumber = document.createElement('div')
        grpSectionNumber.className = 'ppn'
        grpSectionNumber.id = 'ppn' + divCount
        grpArea.appendChild(grpSectionNumber);


        let grpSection = document.createElement('div')
        grpSection.className = 'pp'
        grpSection.id = 'pp' + divCount
        grpArea.appendChild(grpSection);
    ...
    for (let i = 0, nameTemp = nameUnits; i < nameLength.length; i++) {
        ...
        let Randomizer = Math.floor(Math.random() * nameTemp.length);
        groupCounter = groupCounter + 1
        let Transfer = document.getElementById('pp' + groupCounter)
        let TransferNumber = document.getElementById('ppn' + groupCounter)
        if (groupCounter >= grpSet) {
            groupCounter = 0
        }
        ...
        let getNumber = document.getElementById('number' + nameTemp[Randomizer]);
        console.log(getName)
        Transfer.appendChild(getName); // Send to that random grp
        TransferNumber.appendChild(getNumber);
